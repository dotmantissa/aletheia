...
export default function AuctionPage() {
  ...
  useEffect(() => {
    if (created === "1") notify("The seal is formed. Configure your opening narrative and invite bidders.", "success");
  }, [created, notify]);
      ...
      <div className="grid grid-cols-1 gap-6 lg:grid-cols-[1.2fr_0.8fr]">
        <section className="surface p-6">
          ...
          <div className="mt-6 flex flex-wrap items-center gap-3">
            <span className={`inline-flex items-center gap-2 rounded-[4px] border px-2 py-1 text-[11px] ${
              isLive
              ...
              <span className={`h-2 w-2 rounded-full ${isLive ? "bg-[#c8892a]" : isClosedPending ? "bg-[#6b6560]" : "bg-[#c8892a]"}`} />
              {computedStatus}
            </span>
          ...
          <div className="mt-8">
            <div className="h-[2px] w-full bg-[#1a3a5c]">
              <div
                className={`h-full transition-soft ${isSettled ? "bg-[#c8892a] w-full" : isClosedPending ? "bg-[#1a3a5c] w-full animate-pulse" : "bg-[#1a3a5c]"}`}
          ...
          <p className="mt-4 text-xs text-[#6b6560]">Minimum bid floor: {formatLamportsToSol(auction.minBidFloor).toFixed(2)} SOL</p>

          {isClosedPending ? (
            <div className="mt-6 rounded-[4px] border border-[#1a3a5c] bg-[#0d1218] p-4 text-xs text-[#9fb3c8]" style={{ animation: "pulse-sealed 1.9s infinite" }}>
        ...
        <section>
          {arciumLoading ? (
            ...
            <div className="surface p-6 text-xs text-[#7a2a2a]">Arcium channel failed. Retry from trusted network settings.</div>
          ) : (
